...
const getData = async (filters, queries) => {
    ...
    const pageCount = queries.limit ? Math.ceil(totalJobs / queries.limit) : 1;
...
module.exports.addJob = async (req, res, next) => {
    ...
    try {
        const companyName = jobData.company || jobData.comapny;
        const isJobExists = await JobModel.findOne({
            company: companyName,
            position: jobData.position,
        });
        if (isJobExists) {
            return next(createError(400, "Job position at this company already exists"));
        } else {
